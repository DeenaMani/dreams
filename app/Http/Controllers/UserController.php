...
class UserController extends Controller
{
    ...
    public function callbackGoogle()
    {
        try {
            ...
            $google_student = Socialite::driver('google')->user();

            $student = Student::where('google_id', $google_student->getId())->first();

            if(!$student){

                $new_student = Student::create([

                    'first_name' => $google_student->getName(),
                    'email' => $google_student->getEmail(),
                    'google_id' => $google_student->getId(),

                ]);

                session()->put('email', $student->email);
                session()->put('name', $student->first_name." ".$student->last_name);
